...
def calculate_demographic_data(print_data=True):
   ...
   # Drop rows with missing values in 'native-country' or 'salary' columns
    df.dropna(subset=['native-country', 'salary'], inplace=True)

    # Recalculate the counts
    country_count = df['native-country'].value_counts()
    country_rich_count = df[df['salary'] == '>50K']['native-country'].value_counts()

    # Recalculate the highest earning country and its percentage
    ...
    occupation_counts=people_in_india['occupation'].value_counts()
